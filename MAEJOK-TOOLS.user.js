...
(function () {
  ...
  const classes = {
    ...
    add: {
      ...
      modalClose: [
        "close-button_close-button__BKUKA",
        "close-button_sm__n0dZT",
      ],
    },
  };
  ...
  function openSettingsTab(tabName) {
    const tabContents = document.querySelectorAll(
      `[class^="maejok-settings_tab-content"]`
    );
    for (const tabContent of tabContents) {
      tabContent.style.display = "none";
    }
    const content = document.querySelector(
      `.maejok-settings_tab-content_${tabName}`
    );
  ...
  function createSettingsPanel() {
    if (!isPageLoaded || document.querySelector(`.${classes.modalContainer}`))
      return;

    config.load();

    const mainHomeElement = document.querySelector(
      `[class*="${classes.mainHome}"]`
    );
    ...
    const screws = document
      .querySelector(`[class*="${classes.modalScrews}"]`)
      .cloneNode(true);
    ...
    mainHomeElement.parentElement.insertBefore(
      settingsModalContainer,
      mainHomeElement.nextSibling
    );
  ...
  function createSettingsButton() {
    ...
    const chatInputActionsElement = document.querySelector(
      `[class*="${classes.chatInputActions}"]`
    );
    ...
    chatInputActionsElement.insertBefore(
      settingsButton,
      chatInputActionsElement.firstChild
    );

    const settingsButtonImage = document.createElement("img");
    settingsButtonImage.setAttribute(
      "src",
      "/images/slices/console-button-square-red.png"
    );
  ...
  function addSettingsBody(modalBody) {
    ...
    //main
    const enableRecentChatters = createCheckbox(
      "Enable Chatter Count",
      "enableRecentChatters",
      [
        "prompt",
        "Threshold",
        "How long should someone be considered active in chat? (in minutes) (Default: 10)",
        "chattersThreshold",
        "min.",
      ]
    );
    ...
    const enableBigChat = createCheckbox("Big Chat Mode", "enableBigChat", [
      "help",
      " ? ",
      'Hit CTRL+` or CTRL+SHIFT+SPACE to toggle.\nToggles between "MidChat", "BigChat", and Normal.\nToggle once for MidChat, twice for BigChat, and thrice to go back to the original layout',
    ]);
    tabMain.appendChild(enableBigChat);

    const persistBigChat = createCheckbox(
      "Persist Big Chat State",
      "persistBigChat",
      [
        "help",
        " ? ",
        "Remembers which state BigChat is in when you leave the site and returns you to that state on the next visit",
      ]
    );
    tabMain.appendChild(persistBigChat);

    const showTimerBigChat = createCheckbox(
      "Show Countdown in Big Chat Mode",
      "showTimerBigChat",
      [
        "help",
        " ? ",
        "Hides or shows the countdown timer when Big Chat is enabled",
      ]
    );
    tabMain.appendChild(showTimerBigChat);

    const enableClickHighlightUsers = createCheckbox(
      "Enable Users Highlighting",
      "enableClickHighlightUsers",
      [
        "help",
        " ? ",
        "Highlights all messages by user.  Will persist between page reloads.",
      ]
    );
    tabMain.appendChild(enableClickHighlightUsers);

    const enableAvatarTagging = createCheckbox(
      "Avatar Click-To-Tag",
      "enableAvatarTagging",
      ["help", " ? ", "Tags chat sender when you click their username"]
    );
    ...
    const enableDenseChat = createCheckbox(
      "Enable Dense Chat",
      "enableDenseChat",
      ["help", " ? ", "Removes padding around chat messages"]
    );
    ...
    const autoClanChat = createCheckbox("Auto-Join Clan Chat", "autoClanChat", [
      "help",
      " ? ",
      "Automatically joins your clan's chat upon page load",
    ]);
    tabMain.appendChild(autoClanChat);

    //hiders
    const disableTimestamps = createCheckbox(
      "Hide Timestamps",
      "disableTimestamps",
      ["help", " ? ", "Hides timestamps in chat"]
    );
    tabHiders.appendChild(disableTimestamps);

    const disableChatClans = createCheckbox("Hide Clans", "disableChatClans", [
      "help",
      " ? ",
      "Hides clans tags in chat",
    ]);
    tabHiders.appendChild(disableChatClans);

    const disableLevels = createCheckbox("Hide Levels", "disableLevels", [
      "help",
      " ? ",
      "Hides levels in chat",
    ]);
    tabHiders.appendChild(disableLevels);

    const disableAvatars = createCheckbox("Hide Avatars", "disableAvatars", [
      "help",
      " ? ",
      "Hides avatars in chat",
    ]);
    tabHiders.appendChild(disableAvatars);

    const disableEmotes = createCheckbox("Hide Emotes", "disableEmotes", [
      "help",
      " ? ",
      "Hides all emotes in chat",
    ]);
    tabHiders.appendChild(disableEmotes);

    const disableMedals = createCheckbox("Hide Medals", "disableMedals", [
      "help",
      " ? ",
      "Hides all medals in chat.  If only medals are sent, the entire message is removed",
    ]);
    tabHiders.appendChild(disableMedals);

    const disableHappenings = createCheckbox(
      "Hide Happenings",
      "disableHappenings",
      [
        "help",
        " ? ",
        "Hides all notices for consumables (inventory items) and other event notices in chat",
      ]
    );
    tabHiders.appendChild(disableHappenings);

    const disableSystemMessages = createCheckbox(
      "Hide System Messages",
      "disableSystemMessages",
      [
        "help",
        " ? ",
        "Hides all room joins, clan/clanless attacks, War Toys, etc. from chat",
      ]
    );
    tabHiders.appendChild(disableSystemMessages);

    const enableUpdateChecks = createCheckbox(
      "Check for updates",
      "enableUpdateChecks",
      [
        "prompt",
        "Frequency",
        "How many minutes between update checks? (Default: 30) Note: This will also check for new versions on every page load.",
  ...
  function createCheckbox(label, id, prompt = false) {
    ...
    if (prompt) {
      ...
      if (type === "prompt") {
        ...
        const openBracket = document.createTextNode("[");
        const currentValue = document.createTextNode(
          ` (${configValue}${[labelDescriptor]})`
        );
        const closeBracket = document.createTextNode("]");
  ...
  function createListsPane(element, list) {
    ...
    const inputFields = newPane.querySelectorAll(
      `[class^="maejok-settings-lists-list_input"]`
    );
    ...
    bgColor.addEventListener("input", (event) =>
      setListColors(event.target, list, "bg")
    );
    bgAlpha.addEventListener("input", (event) =>
      setListColors(event.target, list, "bg")
    );
    ...
    borderColor.addEventListener("input", (event) =>
      setListColors(event.target, list, "border")
    );
    borderAlpha.addEventListener("input", (event) =>
      setListColors(event.target, list, "border")
    );
  ...
  function setListColors(el, list, bgorbord) {
    ...
    let color_picker_alpha = document.querySelector(
      `#${list}_${bgorbord}_alpha`
    );
    let rgbaColor =
      color_picker.value +
      (color_picker_alpha.value == 255
        ? ""
        : parseInt(color_picker_alpha.value).toString(16).padStart(2, "0"));
  ...
  function addToSettingsList(item, list) {
    const parent = document.getElementById(
      `maejok-settings-lists-${list}-list`
    );
  ...
  function removeFromSettingsList(element, list) {
    const item = element.parentElement.querySelector(
      ".maejok-settings-lists-list_item_name"
    ).textContent;
    if (lists[list].includes(item)) {
      const slideAway =
        list == "friends" ? "slide-away-left" : "slide-away-right";
  ...
  function createFriendButton() {
    ...
    image.src =
      "https://cdn.fishtank.live/images/slices/console-button-long-gray.png";
    image.alt = "";
    image.width = "373";
    image.height = "75";
    ...
    const interval = setInterval(() => {
      ...
      const profile = document.querySelector(
        `[class*="${classes.profileActions}"]`
      );
      if (profile) {
        let users = config.get("friends");
        const username = document.querySelector(
          `[class*="${classes.profileUsername}"]`
        ).innerText;
  ...
  function toggleBigChat(state = null) {
    ...
    const topBarAdmin = document.querySelector(
      `[class*="${classes.topBarAdmin}"]`
    );
    const topBarUser = document.querySelector(
      `[class*="${classes.topBarUser}"]`
    );
    const topBarTitle = document.querySelector(
      `[class*="${classes.topBarTitle}"]`
    );
    const topBarLogo = document.querySelector(
      `[class*="${classes.topBarLogo}"]`
    );
    const happeningMessage = document.querySelector(
      `[class*="${classes.happeningMessage}"]`
    );
    const countDownTimer = document.querySelector(
      `[class*="${classes.countDownTimer}"]`
    );
    const mobileNavPanel = document.querySelector(
      `[class^="${classes.mobileNavPanel}"]`
    );
    const globalMission = document.querySelector(
      `[class^="${classes.globalMission}"]`
    );
    const secondaryPanel = document.querySelector(
      `[class^="${classes.secondaryPanel}"]`
    );
    ...
    if (chatBox) {
      ...
      secondaryPanel.classList.toggle(
        "mTS2-secondaryPanel-BigChat",
        bigChatMode === 2
      );

      chatChatterCount.classList.toggle(
        "mTS2-chatters-BigChat",
        bigChatMode !== 0
      );

      globalMission &&
        globalMission.classList.toggle(
          "mTS2-globalMission-BigChat",
          bigChatMode !== 0
        );

      tryToApplyClass(
        classes.globalMission,
        "mTS2-globalMission-BigChat",
        bigChatMode !== 0
      );
      ...
      if (config.get("showTimerBigChat")) {
        countDownTimer &&
          countDownTimer.classList.remove("mTS2-countdown-BigChat-hide");
        tryToApplyClass(
        ...
        countDownTimer &&
          countDownTimer.classList.remove("mTS2-countdown-BigChat-show");
        tryToApplyClass(
          classes.countDownTimer,
  ...
  function toggleDenseChat(addedNode) {
    ...
    if (config.get("enableDenseChat")) {
      if (!addedNode) {
        ...
        toDensify.forEach((elementName) => {
          const element = addedNode.querySelector(
            `[class*="${classes[elementName]}]"`
          );
          if (element) element.classList.add("mTS2-denseChat");
        });
      ...
      if (chatMessageList)
        chatMessageList.classList.add("mTS2-denseChat-chatMessageList");
    } else {
      toDensify.forEach((elementName) => {
        const elements = document.querySelectorAll(
          `[class*="${classes[elementName]}]"`
      ...
      if (chatMessageList)
        chatMessageList.classList.remove("mTS2-denseChat-chatMessageList");
    }
  }
  ...
  function addMentionedHighlight(mentionedUsername) {
    ...
    const messages = document.querySelectorAll(
      `[data-username="${mentionedUsername}"]`
    );
    if (!messages.length) {
  ...
  function removeMentionedHighlight(sound) {
    ...
    const highlightedMessages = document.querySelectorAll(
      `[class*="maejok-mentioned_highlight"]`
    );
  ...
  function joinClanChat() {
    ...
    const chatRoomSelector = document.querySelector(
      `[class*="${classes.chatRoomSelector}"]`
    );
    const chatRoomButtons = chatRoomSelector.querySelectorAll(
      `[class*="${classes.chatRoomsList}"] button span`
    );
  ...
  function initRecentChatters() {
    ...
    chattersCount.addEventListener("mouseup", (event) => {
      showChattersList(event);
      console.log(event.target);
    });
  }

  function showChattersList(event) {
    const chattersListElm = document.querySelector(
    ...
    if (lists.chatters.length === 0) {
      ...
      lists.chatters.forEach((chatter) => {
        ...
        const username = chatter.user
          ? `<span style="margin-right: 100px;">${chatter.user}</span>`
          : "";
        const clan = chatter.clan
          ? `<span style="position: absolute; right: 6px;">[${chatter.clan}]</span>`
          : "";
        const chatterDiv = document.createElement("div");
        chatterDiv.classList.add(`maejok-chatter_count-chatter`);
  ...
  function updateChattersList(user, clan, array, reset = false) {
    ...
    const thresholdTime =
      currentTime + config.get("chattersThreshold") * 60 * 1000;
  ...
  function handleKeyPress(event) {
    if (!isPageLoaded) return;
    if (
      (event.ctrlKey && event.keyCode === 192) ||
      (event.ctrlKey && event.shiftKey && event.keyCode === 32)
    ) {
      toggleBigChat();
      return;
    }
    if (bigChatMode !== 0 && event.keyCode === 27) {
      toggleBigChat(0);
      return;
    }
    if (event.keyCode === 27) {
  ...
  function handleCloseChattersList(event) {
    ...
    if (
      !hasClass(event.target, "maejok-chatter_count-count") &&
      chattersListElm
    )
      chattersListElm.remove("maejok-chatter_count-chatters-show");
  }
  ...
  function handleHighlightUserEvent(event) {
    ...
    if (parent) {
      const usernameElement = parent.querySelector(
        `[class*="${classes.chatUsername}"]`
      );
      const username = usernameElement
        ? usernameElement.textContent.replace(/\[.*?\]/g, "")
        : false;
  ...
  function handleUserCardButtonEvent(event) {
    const button = document.querySelector(
      `[class*="${classes.userCardActions}"] button`
    );
  ...
  function handleNameTaggingEvent(event) {
    ...
    else if (isChatAvatar && config.get("enableAvatarTagging")) {
      const chatAvatar = event.target.parentElement;
      chatNameEl = chatAvatar.parentElement;
      username = chatNameEl.querySelector(
        `[class*="${classes.chatUsername}"]`
      ).innerHTML;
  ...
  function handleCloseModalEvent(event) {
    ...
    const modalTitle = document.querySelector(
      `[class*="${classes.modalTitle}"]`
    )?.innerText;
    const closeBtn = modalContainer?.querySelector(
      `[class*="${classes.modalCloseButton}"]`
    );
    ...
    const isMT2Modal =
      ((hasClass(event?.target, "close-btn") ||
        event?.target === modalBackdrop) &&
        modalTitle === settingsTitle) ||
      false;
    const isBGOrCloseBtn =
      event?.target === modalBackdrop || event?.target === closeBtn || false;
  ...
  function splitHexA(hexWithOpacity) {
    ...
    if (!hexPattern.test(hexWithOpacity))
      return { hex: "#000000", number: 255 };
  ...
  function tryToApplyClass(selector, classToApply, forceToggle) {
    ...
    const interval = setInterval(() => {
      ...
      if (element || count * 5 >= duration) {
        // try for 15 seconds in case shit is slow loading
  ...
  function checkForUpdate() {
    checkVersion((hasUpdate, newVersion) => {
      if (!hasUpdate && upToDateNoticeShown) {
        ...
        console.error(
          `\x1b[32mMAEJOK-TOOLS v${VERSION} :: UPDATE AVAILABLE!  Newest version: ${newVersion}`
        );
        console.error(
          `\x1b[32mHead on over to https://greasyfork.org/en/scripts/465416-maejok-tools-for-fishtank-live/ to update!`
        );

        insertChatMessage(
          `<div onclick="(event.target.tagName !== 'A') ? (document.getElementById('maejok-update').innerHTML = '<h4 style=\\'color:#F04;animation:notice-me 2s infinite;\\'>MAEJOK LOVES YOU!</h4><p style=\\'font-size:.7rem;color:#fff\\'>Refresh to start using the new version</p>',window.open('https://greasyfork.org/en/scripts/465416-maejok-tools-for-fishtank-live/','_blank')) : null" style="animation:notice-me.3s 7;margin:4px;cursor:pointer;margin-top:10px;text-align:center;padding:10px"><div id="maejok-update"><h2 style="font-size:1.2rem;margin:0;color:#f07">MAEJOK-TOOLS</h2><p style="font-size:1rem;color:#fff"><b>Click to Update</b></p></div><div style="text-align:center;margin:5px 0 5px;font-size:.8rem;color:#fff">[ <a class="maejok-update" href="https://github.com/maejok-xx/MAEJOK-TOOLS-FISHTANK/blob/main/changelog.md" target="_blank">view change log</a> - <a class="maejok-update" href="#" onclick="event.preventDefault();document.getElementById('maejok-update').parentElement.remove();">dismiss</a> ]</div></div>`
        );
  ...
  function startUpdateChecker() {
    if (!config.get("enableUpdateChecks")) {
      if (!updateCheckIntervalId)
        console.log("MAEJOK-TOOLS :: Update Checking Disabled");
      ...
      updateCheckIntervalId = setInterval(
        checkForUpdate,
        config.get("updateCheckFrequency") * 60 * 1000
      );
    }
  }
  ...
  function checkVersion(callback) {
    ...
    fetch(
      `https://raw.githubusercontent.com/maejok-xx/MAEJOK-TOOLS-FISHTANK/main/version.txt?cb=${ts}`
    )
      .then((response) => {
        if (!response.ok) {
          throw new Error(
            `MAEJOK-TOOLS :: Update Checker failed with status: ${response.status}`
          );
        }
  ...
  function initChatMutationObserver() {
    ...
    if (configChecks.every((check) => !config.get(check))) {
      chatMutationObserver?.disconnect();
    }
    ...
    if (!config.get("enableDenseChat")) {
      document
        .querySelectorAll('[class*="mTS2-denseChat"]')
    ...
    const options = [
      {
        selector: `[class*="${classes.chatClan}"]`,
        condition: config.get("disableChatClans"),
      },
      {
        selector: `[class*="${classes.chatTimestamps}"]`,
        condition: config.get("disableTimestamps"),
      },
      {
        selector: `[class*="${classes.chatXPLevel}"]`,
        condition: config.get("disableLevels"),
      },
      {
        selector: `[class*="${classes.chatAvatar}"] img`,
        condition: config.get("disableAvatars"),
      },
      {
        selector: `[class*="${classes.chatEmote}"]`,
        condition: config.get("disableEmotes"),
      },
      {
        selector: `[class*="${classes.chatSystem}"]`,
        condition: config.get("disableSystemMessages"),
      },
      {
        selector: `[class*="${classes.chatHappening}"]`,
        condition: config.get("disableHappenings"),
      },
    ];
    ...
    options.forEach((option) => {
      document.querySelectorAll(option.selector).forEach((element) => {
        option.condition
          ? element.classList.add("mTS2-chat-hide")
          : element.classList.remove("mTS2-chat-hide");
      });
    });

    // HIGHLIGHT
    const messages = document.querySelectorAll(
      `[class*="${classes.chatMessageOuter}"]`
    );
    messages.forEach((element) => {
      const userElm = element.querySelector(
        `[class*="${classes.chatUsername}"]`
      );
      let username = "";
      if (userElm) {
        ...
        if (lists.friends.includes(username)) {
          ...
          element.style.border = `1px solid ${
            config.get("colors").friends.border
          }`;
        } else {
        ...
        if (lists.watching.includes(username)) {
          ...
          element.style.border = `1px solid ${
            config.get("colors").watching.border
          }`;
        } else {
          if (!lists.friends.includes(username))
            element.style.backgroundColor = "";
    ...
    chatMutationObserver = new MutationObserver(function (
      ...
      mutationsList.forEach((mutation) => {
        ...
        mutation.addedNodes.forEach((addedNode) => {
          ...
          const mentions = addedNode.querySelectorAll(
            `[class^="${classes.chatMention}"]`
          );
          ...
          const userElm = addedNode.querySelector(
            `[class*="${classes.chatUsername}"]`
          );
          const clanElm = addedNode.querySelector(
            `[class*="${classes.chatUsername}"] span`
          )?.innerHTML;
          let username = "";
          let clan = "";
          if (userElm) {
            ...
            if (lists.friends.includes(username)) {
              addedNode.style.backgroundColor =
                config.get("colors").friends.color;
              addedNode.style.border = `1px solid ${
                config.get("colors").friends.border
              }`;
            }
            if (lists.watching.includes(username)) {
              addedNode.style.backgroundColor =
                config.get("colors").watching.color;
              addedNode.style.border = `1px solid ${
                config.get("colors").watching.border
              }`;
            }
          ...
          let systemMessage = addedNode.querySelector(
            `[class*="${classes.chatSystem}"] div`
          );
          if (systemMessage) {
            const msg = systemMessage.textContent;
            if (msg.includes("Joined "))
              updateChattersList(my.name, my.clan, lists.chatters, true);
          }
          // end clear chatters

          let usernameColor = null;
          const chatXPLevel = addedNode.querySelector(
            `[class*="${classes.chatXPLevel}"]`
          );
          const chatUsername = addedNode.querySelector(
            `[class*="${classes.chatUsername}"]`
          );

          // DENSE CHAT
          if (chatUsername)
            usernameColor = chatUsername.getAttribute("style") || null;
          if (chatUsername && config.get("disableAvatars")) {
            chatUsername.style.cssText = `${usernameColor}${
              chatXPLevel && !config.get("disableLevels")
                ? `margin-left: ${chatXPLevel.offsetWidth + 2}px;`
                : ""
            }`;
          }
          if (chatXPLevel && config.get("disableAvatars"))
            chatXPLevel.classList.add(
          ...
          const options = [
            ...
            {
              selector: `[class*="${classes.chatMedal}"]`,
              condition: config.get("disableMedals"),
            },
            {
              selector: `[class*="${classes.chatClan}"]`,
              condition: config.get("disableChatClans"),
            ...
            {
              selector: `[class*="${classes.chatSystem}"] div`,
              condition: config.get("disableSystemMessages"),
            },
            {
              selector: `[class*="${classes.chatHappening}"]`,
          ...
          options.forEach((option) => {
            if (option.condition) {
              ...
              if (
                option.selector === system &&
                addedNode.querySelector(system)
              ) {
                addedNode.classList.add("mTS2-chat-hide");
              } else {
                addedNode
                  .querySelectorAll(option.selector)
                  .forEach((element) =>
                    element.classList.add("mTS2-chat-hide")
                  );
              }
              ...
              if (
                option.condition &&
                option.selector === `[class*="${classes.chatMedal}"]`
              ) {
                const message = addedNode.querySelector(
                  `[class*="${classes.chatMessage}"]`
                );
    ...
    chatMutationObserver.observe(chatMessageList, {
      attributes: false,
      childList: true,
      subtree: false,
  ...
  const initConfig = () => {
    ...
    const load = () => {
      const storedSettings = JSON.parse(
        localStorage.getItem("s2_maejoktools-settings")
      );
      if (!storedSettings) return;

      if (
        !storedSettings.colors ||
        Object.keys(storedSettings.colors).length === 0 ||
        storedSettings.colors.v !== 1
      ) {
        storedSettings.colors = { ...settings.colors };
      }

      for (const key in storedSettings)
        settings[key] = settings.hasOwnProperty(key) && storedSettings[key];
  ...
  function start() {
    ...
    const initWait = setInterval(() => {
      const waitElement = document.querySelector(
        `[class^="${config.internal("waitElement")}`
      );
      if (waitElement) {
        ...
        sessionStorage.getItem("maejok-showChangelog")
          ? insertChangelog()
          : null;

        if (config.get("persistBigChat") && config.get("bigChatState"))
          toggleBigChat(config.get("bigChatState"));
        window.addEventListener("resize", handleWindowSize);
      }

      const chatMessagesElement = document.querySelector(
        `[class*="${classes.chatMessages}"]`
      );
  ...
  function runAgreement() {
    ...
    if (agreement === "yes sir") {
      ...
      alert(
        "You did not accept the agreement.  MAEJOK-TOOLS will not load.\nDisable the MAEJOK-TOOLS plugin to disable this alert."
      );
    }
  }
